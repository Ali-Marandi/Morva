...
def _evidence(**overrides):
    payload = {
        ...
        "evidence_document_sha256": EVIDENCE_DOC,
...
def test_hash_mismatch_is_rejected():
    with pytest.raises(ReconciliationEvidenceError, match="evidence document SHA-256"):
        build_three_way_reconciliation_binding(
            _evidence(evidence_document_sha256="f" * 64),
            _registry(),
            bound_by="reconciliation-binder",
            bound_at=NOW,
        )
