  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      ...
      {profile.data?.data && (
        <section className="bg-white rounded-xl border border-morva-200 p-6 shadow-sm">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            <div><span className="text-morva-500">نام</span><div className="font-semibold mt-1">{profile.data.data.first_name} {profile.data.data.last_name}</div></div>
            <div><span className="text-morva-500">کد کارمند</span><div className="font-semibold mt-1">{profile.data.data.employee_no}</div></div>
            <div><span className="text-morva-500">وضعیت</span><div className="font-semibold mt-1">{profile.data.data.status}</div></div>
          </div>
        </section>
      )}
      ...
      <section className="bg-white rounded-xl border border-morva-200 p-6 shadow-sm">
        ...
        {!payslips.isLoading && !payslips.isError && (payslips.data?.data ?? []).length === 0 && <p className="text-sm text-morva-600">فیش قابل نمایش وجود ندارد.</p>}
      ...
      <section className="bg-white rounded-xl border border-morva-200 p-6 shadow-sm">
        ...
        {!orders.isLoading && !orders.isError && (orders.data?.data ?? []).length === 0 && <p className="text-sm text-morva-600">حکم قابل نمایش وجود ندارد.</p>}
        {(orders.data?.data ?? []).length > 0 && <div className="space-y-3">{(orders.data?.data ?? []).map((order) => <div key={order.order_no} className="rounded-lg border border-morva-100 p-4"><div className="font-semibold">{order.order_no} — {order.order_type}</div><div className="text-xs text-morva-600 mt-1">تاریخ اجرا: {order.effective_date}</div><div className="text-sm mt-2">{order.reason || 'بدون شرح ثبت‌شده'}</div></div>)}</div>}
      </section>
    </div>
  );
}
