...
SOURCES = {
    "payroll": "گزارش لیست حقوق.xlsx",
    "supplementary": "اکسل گزارش بیمه تکمیلی.xlsx",
    "loans": "اکسل گزارش کسر اقساط (1).xlsx",
    "orders": "گزارش احکام حقوقی.xlsx",
    "health": "گزارش بیمه خدمات درمانی.xlsx",
    "social": "گزارش لیست بیمه تامین اجتماعی.xlsx",
}

GROSS_COLUMNS = (
    "حق شغل-1", "حق شاغل-2", "تفاوت  تطبيق-3", "فوق العاده سختی کار-4", "فوق العاده بدی آب و هوا-7",
    "کمک هزینه عائله مندی-11", "کمک هزینه اولاد-12", "حداقل  دريافتي-13", "فوق العاده رتبه بندی-18",
    "ترمیم حقوق-19-19", "فوق العاده شغل-22", "سایر-23", "فوق العاده ایثارگری ماده 51-27",
    "تفاوت تطبیق موضوع جزء(2-1)بندالف تبصره15 ق ب 1403-50", "فوق العاده مديريت-57",
    "فوق العاده ی مناطق کمتر توسعه یافته-58", "فوق العاده ویژه-66", "حق لباس (سربازمعلم )-72",
    "فوق العاده ایثارگری-91", "تفاوت بند(ی) تبصره (12) ق.ج-95", "تفاوت جزء(1) بند(الف)تبصره(12) ق. ج-96",
    "فوق العاده تخصصی-99", "حق التدریس-110", "فوق العاده مشاغل خدماتی-113", "فوق العاده خاص-129",
    "برگشت مامورین خارج از کشور - بعد از 1404-131", "بازگشت بیمه تکمیلی-160",
)
...
class MorvaImportService:
    ...
    def import_directory(self, directory: str | Path, source_period: str = "1405-05") -> tuple[ImportReport, dict[str, list[ImportRecord]]]:
        ...
        for row in payroll:
            employee = key("EMP", row["کد پرسنلی"])
            bundle["payroll"].append(ImportRecord(employee, SOURCES["payroll"], source_period, {"employee_key": employee, "national_key": key("NID", row["کد ملی"]), "gross": str(money(row["جمع مزایا"])), "deductions": str(money(row["جمع کسور"])), "net": str(money(row["خالص پرداختی"]))}))
