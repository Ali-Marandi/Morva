...
def materialize_import(
    ...
    *,
    ...
    batch = ImportBatchRecord(
        ...
        status="quarantined" if report.critical_exception_count else "ready",
    ...
    for record in records:
        ...
        session.add(
            ImportRecordRecord(
                ...
                source_employee_key=record.employee_key,
        ...
        if employee is None:
            session.add(
                ImportIssueRecord(
                    ...
                    severity="critical",
                    record_key=record.employee_key,
                    evidence={"source": record.source, "period": record.period},
                    status="quarantined",
                )
            )
        ...
        existing_snapshot = session.scalar(
            select(PersonnelSnapshotRecord).where(
                PersonnelSnapshotRecord.employee_no == employee.employee_no,
    ...
    append_audit_event(
        ...
        reason="materialize immutable source import",
        session=session,
    )
    return batch
