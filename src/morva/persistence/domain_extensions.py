...
class LifecycleEventRecord(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    __table_args__ = (
        UniqueConstraint("payroll_run_id", "sequence_no", name="uq_payroll_lifecycle_sequence"),
        UniqueConstraint("payroll_run_id", "idempotency_key", name="uq_payroll_lifecycle_idempotency"),
    )
...
class EmployeeCaseRecord(Base):
    ...
    title: Mapped[str] = mapped_column(String(200), default="درخواست کارکنان")
    description: Mapped[str] = mapped_column(Text, default="")
    priority: Mapped[str] = mapped_column(String(20), default="normal", index=True)
    submitted_by: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
