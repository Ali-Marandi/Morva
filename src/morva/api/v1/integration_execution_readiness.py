...
from morva.runtime.independent_historical_m4_66_receipt_verifier_m4_67 import (
    ...
    independently_verify_historical_m4_66_receipt as verify_m4_66_receipt_independently,
)
...
class IndependentHistoricalM466ReceiptVerificationResponse(BaseModel):
    verification: dict[str, object]
...
class IndependentHistoricalM464ReceiptHistoryIntegrityReceiptResponse(BaseModel):
    id: UUID
    snapshot_id: UUID
    verification: dict[str, object]
    recorded_by: str
...
class IndependentHistoricalM464ReceiptHistoryIntegrityReceiptHistoryResponse(BaseModel):
    items: list[IndependentHistoricalM464ReceiptHistoryIntegrityReceiptResponse]
    has_more: bool
    next_before_created_at: datetime | None = None
    next_before_id: UUID | None = None
...
class IndependentHistoricalM466VerificationPersistenceReceiptResponse(BaseModel):
    id: UUID
    verification_receipt_id: UUID
    verification: dict[str, object]
    recorded_by: str
...
class IndependentHistoricalM466VerificationPersistenceReceiptHistoryResponse(BaseModel):
    items: list[IndependentHistoricalM466VerificationPersistenceReceiptResponse]
    has_more: bool
    next_before_created_at: datetime | None = None
    next_before_id: UUID | None = None
...
@router.post(
    ...
    response_model=IndependentHistoricalM464ReceiptHistoryIntegrityReceiptResponse,
)
def persist_independent_historical_m4_64_receipt_history_integrity_receipt(
    snapshot_id: UUID,
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM464ReceiptHistoryIntegrityReceiptResponse:
    authorize(
        principal,
        "evidence.binding.write",
        principal.scope,
    ...
    if principal.scope is not Scope.MINISTRY:
        raise HTTPException(
            status_code=403,
            detail="M4.66 verification receipts are ministry-managed",
        )
    with SessionLocal() as session:
        ...
        try:
            ...
            append_audit_event(
                event_type=(
                    "integration.readiness."
                    "independent_historical_m4_64_receipt_history_integrity."
                    "receipt.recorded"
                ),
                entity_type="independent_historical_m4_64_receipt_history_integrity_receipt",
                entity_id=str(record.id),
                actor_id=principal.user_id,
                payload={
                    "snapshot_id": str(record.snapshot_id),
                ...
                reason="M4.66 independent M4.65 verification receipt persisted",
                session=session,
            )
            session.commit()
            ...
            status = 404 if "not found" in str(exc) else 409
            raise HTTPException(status_code=status, detail=str(exc)) from exc
    return IndependentHistoricalM464ReceiptHistoryIntegrityReceiptResponse(
        id=record.id,
        snapshot_id=record.snapshot_id,
        verification=verification.to_payload(),
        recorded_by=record.recorded_by,
...
@router.get(
    ...
    response_model=IndependentHistoricalM464ReceiptHistoryIntegrityReceiptHistoryResponse,
)
def list_independent_historical_m4_64_receipt_history_integrity_receipts(
    snapshot_id: UUID | None = Query(default=None),
    valid: bool | None = Query(default=None),
    before_created_at: datetime | None = Query(default=None),
    before_id: UUID | None = Query(default=None),
    limit: int = Query(default=50, ge=1, le=100),
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM464ReceiptHistoryIntegrityReceiptHistoryResponse:
    authorize(principal, "evidence.read", principal.scope)
    if principal.scope is not Scope.MINISTRY:
        raise HTTPException(
            status_code=403,
    ...
    return IndependentHistoricalM464ReceiptHistoryIntegrityReceiptHistoryResponse(
        items=items,
        has_more=has_more,
        next_before_created_at=records[-1].created_at
        if has_more and records
...
def verify_independent_historical_m4_64_receipt_history_integrity_receipt(
    verification_receipt_id: UUID,
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM464ReceiptHistoryIntegrityReceiptResponse:
    authorize(principal, "evidence.read", principal.scope)
...
@router.get(
    ...
    response_model=IndependentHistoricalM466ReceiptVerificationResponse,
)
def independently_verify_historical_m4_66_receipt(
    verification_receipt_id: UUID,
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM466ReceiptVerificationResponse:
    authorize(principal, "evidence.read", principal.scope)
    with SessionLocal() as session:
        receipt = session.get(
            IndependentHistoricalM464ReceiptHistoryIntegrityReceiptRecord,
        ...
        if receipt is None:
            raise HTTPException(
                status_code=404,
                detail="M4.66 verification receipt not found",
            )
        snapshot = session.get(
            HistoricalM463ReceiptHistoryIntegrityRecord,
            receipt.snapshot_id,
        ...
        try:
            ...
            for source_record in source_records:
                source_repository.verify(source_record.id)
            verification = verify_m4_66_receipt_independently(
                receipt=receipt,
                snapshot=snapshot,
                source_records=source_records,
            )
            ...
            raise HTTPException(status_code=409, detail=str(exc)) from exc
    return IndependentHistoricalM466ReceiptVerificationResponse(
        verification=verification.to_payload(),
    )
...
@router.post(
    ...
    response_model=IndependentHistoricalM466VerificationPersistenceReceiptResponse,
)
def persist_independent_historical_m4_67_verification(
    verification_receipt_id: UUID,
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM466VerificationPersistenceReceiptResponse:
    authorize(
        principal,
        "evidence.binding.write",
        principal.scope,
    ...
    if principal.scope is not Scope.MINISTRY:
        raise HTTPException(
            status_code=403,
            detail="M4.68 verification results are ministry-managed",
        )
    with SessionLocal() as session:
        ...
        try:
            ...
            append_audit_event(
                event_type=(
                    "integration.readiness."
                    "independent_historical_m4_67_verification."
                    "result.recorded"
                ),
                entity_type="independent_historical_m4_67_verification_result",
                entity_id=str(record.id),
                actor_id=principal.user_id,
                payload={
                ...
                reason="M4.68 independent M4.67 verification result persisted",
                session=session,
            )
            session.commit()
        except (
            ...
            status = 404 if "not found" in str(exc) else 409
            raise HTTPException(status_code=status, detail=str(exc)) from exc
    return IndependentHistoricalM466VerificationPersistenceReceiptResponse(
        id=record.id,
...
@router.get(
    ...
    response_model=IndependentHistoricalM466VerificationPersistenceReceiptHistoryResponse,
)
def list_independent_historical_m4_67_verification_results(
    ...
    limit: int = Query(default=50, ge=1, le=100),
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM466VerificationPersistenceReceiptHistoryResponse:
    authorize(principal, "evidence.read", principal.scope)
    if principal.scope is not Scope.MINISTRY:
        raise HTTPException(
            status_code=403,
    ...
    with SessionLocal() as session:
        ...
        except IndependentHistoricalM466VerificationPersistenceReceiptPersistenceError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
    items = [
        IndependentHistoricalM466VerificationPersistenceReceiptResponse(
            id=record.id,
    ...
    return IndependentHistoricalM466VerificationPersistenceReceiptHistoryResponse(
        items=items,
        has_more=has_more,
        next_before_created_at=records[-1].created_at if has_more and records else None,
        next_before_id=records[-1].id if has_more and records else None,
...
def verify_independent_historical_m4_67_verification_result(
    verification_id: UUID,
    principal: Principal = Depends(get_current_principal),
) -> IndependentHistoricalM466VerificationPersistenceReceiptResponse:
    authorize(principal, "evidence.read", principal.scope)
