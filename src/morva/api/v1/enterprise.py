...
def materialize_artifacts(run_id: UUID, principal: Principal = Depends(get_current_principal)) -> dict[str, object]:
    with SessionLocal() as session:
        ...
        if not run.ruleset_hash:
            raise HTTPException(status_code=423, detail="payroll run has no immutable rule-pack hash")
        count = materialize_run_artifacts(run.id)
        append_audit_event(
            event_type="payroll.artifacts.materialized",
            entity_type="payroll_run",
            entity_id=str(run.id),
            actor_id=principal.user_id,
            payload={"created": count},
            reason="persist deterministic employee payroll artifacts",
            session=session,
        )
        session.commit()
...
def create_payment_batch(
    ...
    with SessionLocal() as session:
        ...
        if not run.approved_by or run.approved_by == principal.user_id:
        ...
        batch = PaymentBatchRecord(
            id=uuid4(), payroll_run_id=run.id, batch_reference=f"MORVA-{run.period}-{uuid4().hex[:12].upper()}",
            currency_code=run.currency_code, amount=Decimal(artifact_sum), beneficiary_count=int(artifact_count),
            status="approved_pending_submission", created_by=principal.user_id,
        )
        session.add(batch)
        session.flush()
        enqueue(
            ...
            payload={"payment_batch_id": str(batch.id), "batch_reference": batch.batch_reference, "amount": str(batch.amount), "beneficiary_count": batch.beneficiary_count, "currency_code": batch.currency_code},
        )
        append_audit_event(
            event_type="payment.batch.created",
            entity_type="payment_batch",
            entity_id=str(batch.id),
            actor_id=principal.user_id,
            payload={"payroll_run_id": str(run.id), "batch_reference": batch.batch_reference, "amount": str(batch.amount), "beneficiary_count": batch.beneficiary_count},
            reason="create payment batch after frozen payroll approval",
            session=session,
        )
        session.commit()
