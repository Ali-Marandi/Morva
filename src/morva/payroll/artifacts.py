...
def _canonical_result_payload(employee_no: str, period: str, ruleset_version: str, snapshot_hash: str, lines: Iterable[PayrollLine]) -> str:
    payload = {
        ...
        "lines": [
            {"code": line.code, "title": line.title, "amount": str(line.amount), "kind": line.kind,
...
def materialize_run_artifacts(session: Session, run_id: UUID) -> int:
    """Persist employee-level deterministic results in the caller's transaction."""
    ...
    for employee_no in employees:
        ...
        existing = session.scalar(select(PayrollArtifactRecord).where(PayrollArtifactRecord.payroll_run_id == run.id, PayrollArtifactRecord.employee_no == employee_no))
        if existing is not None:
            continue
        employee_lines = [
            PayrollLine(code=line.code, title=line.title, amount=Decimal(line.amount), kind=line.kind,
                        taxable=line.taxable, pensionable=line.pensionable, insurable=line.insurable,
                        rule_code=line.rule_code, explanation=line.explanation)
            for line in lines if line.employee_no == employee_no
        ]
        calculation = calculator.calculate(employee_no=employee_no, period=run.period, ruleset_version=run.ruleset_version, lines=employee_lines)
        input_hash = hashlib.sha256(_canonical_result_payload(employee_no, run.period, run.ruleset_version, snapshot.snapshot_hash, employee_lines).encode("utf-8")).hexdigest()
        output_hash = hashlib.sha256(json.dumps({"gross": str(calculation.result.gross), "deductions": str(calculation.result.deductions), "net": str(calculation.result.net), "fingerprint": calculation.fingerprint}, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
        artifact = PayrollArtifactRecord(id=uuid4(), payroll_run_id=run.id, employee_no=employee_no, period=run.period,
            currency_code=run.currency_code, personnel_snapshot_id=snapshot.id, personnel_snapshot_hash=snapshot.snapshot_hash,
            rule_pack_version=run.ruleset_version, rule_pack_hash=run.ruleset_hash, input_hash=input_hash,
            output_hash=output_hash, gross=calculation.result.gross, deductions=calculation.result.deductions,
