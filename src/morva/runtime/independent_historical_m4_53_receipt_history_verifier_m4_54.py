...
class IndependentHistoricalM453ReceiptHistoryIntegrityError(ValueError):
    """Raised when an M4.53 receipt-history snapshot fails independent verification."""


@dataclass(frozen=True, slots=True)
class IndependentHistoricalM453ReceiptHistoryIntegrity:
    snapshot_id: UUID
    ...
    def __post_init__(self) -> None:
        for name, value in (
            ...
            if len(value) != 64 or any(
                ...
                raise IndependentHistoricalM453ReceiptHistoryIntegrityError(
        ...
        if self.verification_fingerprint.lower() != expected:
            raise IndependentHistoricalM453ReceiptHistoryIntegrityError(
                "independent M4.53 receipt-history verification fingerprint mismatch"
            )
...
def independently_verify_historical_m4_53_receipt_history_integrity(
    *,
    ...
    try:
        ...
        raise IndependentHistoricalM453ReceiptHistoryIntegrityError(
            "persisted M4.53 receipt-history integrity snapshot is structurally invalid"
        ) from exc
