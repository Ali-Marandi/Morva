...
class EvidenceRemediationItem:
    ...
    def __post_init__(self) -> None:
        if self.role not in CLOSURE_ROLE_SOURCE_TYPES:
            raise EvidenceReadinessError(f"unsupported certification role: {self.role}")
        if self.state not in {"ready", "blocked"}:
            raise EvidenceReadinessError(f"unsupported remediation state: {self.state}")
        if not self.reason_code.strip() or not self.detail.strip():
            raise EvidenceReadinessError("reason_code and detail are required")
...
class EvidenceReadinessAssessment:
    ...
    def __post_init__(self) -> None:
        if self.assessment_version != 1:
            raise EvidenceReadinessError("unsupported evidence readiness assessment version")
        if not self.repository.strip():
            raise EvidenceReadinessError("repository is required")
        if self.checked_at.tzinfo is None:
            raise EvidenceReadinessError("checked_at must be timezone-aware")
        ...
        if set(self.ready_roles) & set(self.blocked_roles):
            raise EvidenceReadinessError("role cannot be both ready and blocked")
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            ...
            "remediation": [item.to_payload() for item in self.remediation],
...
def build_readiness_assessment(
    ...
    *,
    ...
    receipt_by_role = {receipt.certification_role: receipt for receipt in receipts}
    registry_by_id = {item.evidence_id: item for item in registry.items}
    superseded = set(lifecycle.superseded_evidence_ids) if lifecycle else set()
        ...
        if receipt is None:
            ...
            remediation.append(
                EvidenceRemediationItem(
                    ...
                    detail="Create a current role binding against the active evidence registry.",
                )
            )
            continue
        ...
        if evidence is None:
            ...
            remediation.append(
                EvidenceRemediationItem(
                    ...
                    detail="The bound authoritative evidence is no longer present in the current registry.",
        ...
        if receipt.authoritative_evidence_id in superseded:
            ...
            remediation.append(
                EvidenceRemediationItem(
                    ...
                    detail="Rebind this certification role to the current lineage head.",
                    evidence_id=receipt.authoritative_evidence_id,
                )
            )
            continue
        ...
        if receipt.population_scope.strip() != evidence.population_scope.strip():
            ...
            remediation.append(
                EvidenceRemediationItem(
                    ...
                    detail="Rebind using evidence with the exact certified population scope.",
                    evidence_id=receipt.authoritative_evidence_id,
                )
            )
            continue
    ...
    fingerprint = _readiness_fingerprint(
        ...
        lifecycle_fingerprint=lifecycle.fingerprint if lifecycle else None,
        ready_roles=tuple(ready_roles),
...
def _readiness_fingerprint(
    *,
    ...
    payload = {
        ...
        "lifecycle_fingerprint": lifecycle_fingerprint.lower()
        if lifecycle_fingerprint
        else None,
        "ready_roles": list(ready_roles),
        "blocked_roles": list(blocked_roles),
        "remediation": [item.to_payload() for item in remediation],
    }
    return sha256(
        json.dumps(
            payload,
