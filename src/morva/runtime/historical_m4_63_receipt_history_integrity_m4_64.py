...
class HistoricalM463ReceiptHistoryIntegrityError(ValueError):
    """Raised when an M4.64 M4.63 receipt-history integrity result is invalid."""


@dataclass(frozen=True, slots=True)
class HistoricalM463ReceiptHistoryIntegrity:
    ...
    def __post_init__(self) -> None:
        if self.integrity_version != 1:
            raise HistoricalM463ReceiptHistoryIntegrityError(
                "unsupported M4.64 receipt-history integrity version"
            )
        for name, value in (
        ...
        if self.fingerprint.lower() != expected:
            raise HistoricalM463ReceiptHistoryIntegrityError(
                "M4.64 receipt-history integrity fingerprint mismatch"
            )
...
def build_historical_m4_63_receipt_history_integrity(
    records: list[IndependentHistoricalM461ReceiptHistoryIntegrityReceiptRecord],
