...
class IndependentHistoricalM458ReceiptHistoryIntegrityError(ValueError):
    """Raised when an M4.58 receipt-history snapshot fails independent verification."""


@dataclass(frozen=True, slots=True)
class IndependentHistoricalM458ReceiptHistoryIntegrity:
    snapshot_id: UUID
    ...
    def __post_init__(self) -> None:
        for name, value in (
            ...
            if len(value) != 64 or any(char not in "0123456789abcdef" for char in value.lower()):
                raise IndependentHistoricalM458ReceiptHistoryIntegrityError(f"{name} must be SHA-256")
        for name, value in (
            ...
            if value < 0:
                raise IndependentHistoricalM458ReceiptHistoryIntegrityError(f"{name} cannot be negative")
        ...
        if self.verification_fingerprint.lower() != expected:
            raise IndependentHistoricalM458ReceiptHistoryIntegrityError(
                "independent M4.58 receipt-history verification fingerprint mismatch"
            )
...
def independently_verify_historical_m4_58_receipt_history_integrity(
    *,
    ...
    try:
        ...
        raise IndependentHistoricalM458ReceiptHistoryIntegrityError(
            "persisted M4.58 receipt-history integrity snapshot is structurally invalid"
        ) from exc
    point_in_time_records = [
        record for record in source_records
        if _timestamp(record.created_at) < _timestamp(snapshot.created_at)
    ]
    ordered = sorted(point_in_time_records, key=lambda record: (_timestamp(record.created_at), str(record.id)))
    ...
    for record in ordered:
        try:
            ...
            raise IndependentHistoricalM458ReceiptHistoryIntegrityError(
                "M4.57 source verification receipt is structurally invalid"
            ) from exc
    ...
    if persisted.integrity_version != 1:
        blockers.append("M458_INTEGRITY_VERSION_MISMATCH")
    if persisted.history_fingerprint.lower() != reconstructed_history_fingerprint:
        blockers.append("M458_HISTORY_FINGERPRINT_MISMATCH")
    if persisted.record_count != reconstructed_record_count:
        blockers.append("M458_RECORD_COUNT_MISMATCH")
    if persisted.valid_count != valid_count:
        blockers.append("M458_VALID_COUNT_MISMATCH")
    if persisted.fingerprint.lower() != reconstructed_fingerprint:
        blockers.append("M458_INTEGRITY_FINGERPRINT_MISMATCH")
...
def _history_fingerprint(*, record_count: int, valid_count: int, history_fingerprint: str) -> str:
    payload = {
        "integrity_version": 1,
