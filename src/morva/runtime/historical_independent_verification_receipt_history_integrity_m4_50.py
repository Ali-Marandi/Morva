...
class HistoricalIndependentVerificationReceiptHistoryIntegrityError(ValueError):
    """Raised when an M4.50 receipt-history integrity result is invalid."""


@dataclass(frozen=True, slots=True)
class HistoricalIndependentVerificationReceiptHistoryIntegrity:
    ...
    def __post_init__(self) -> None:
        if self.integrity_version != 1:
            raise HistoricalIndependentVerificationReceiptHistoryIntegrityError(
                "unsupported M4.50 receipt-history integrity version"
            )
        for name, value in (
        ...
        if self.fingerprint.lower() != expected:
            raise HistoricalIndependentVerificationReceiptHistoryIntegrityError(
                "receipt-history integrity fingerprint mismatch"
            )
...
def build_historical_independent_verification_receipt_history_integrity(
    records: list[
        IndependentHistoricalFreshnessVerificationHistoryIntegrityReceiptRecord
    ],
