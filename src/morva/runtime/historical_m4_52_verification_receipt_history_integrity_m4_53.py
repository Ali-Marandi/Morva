...
class HistoricalM452VerificationReceiptHistoryIntegrityError(ValueError):
    """Raised when an M4.53 M4.52 receipt-history integrity result is invalid."""


@dataclass(frozen=True, slots=True)
class HistoricalM452VerificationReceiptHistoryIntegrity:
    ...
    def __post_init__(self) -> None:
        if self.integrity_version != 1:
            raise HistoricalM452VerificationReceiptHistoryIntegrityError(
                "unsupported M4.53 receipt-history integrity version"
            )
        for name, value in (
        ...
        if self.fingerprint.lower() != expected:
            raise HistoricalM452VerificationReceiptHistoryIntegrityError(
                "M4.53 receipt-history integrity fingerprint mismatch"
            )
...
def build_historical_m4_52_verification_receipt_history_integrity(
    records: list[
        IndependentHistoricalVerificationReceiptHistoryIntegrityReceiptRecord
    ],
