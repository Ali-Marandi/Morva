...
class HistoricalM468VerificationHistoryIntegrityError(ValueError):
    """Raised when an M4.69 M4.68 history-integrity result is invalid."""


@dataclass(frozen=True, slots=True)
class HistoricalM468VerificationHistoryIntegrity:
    ...
    def __post_init__(self) -> None:
        if self.integrity_version != 1:
            raise HistoricalM468VerificationHistoryIntegrityError(
                "unsupported M4.69 verification-history integrity version"
            )
        for name, value in (
        ...
        if self.fingerprint.lower() != expected:
            raise HistoricalM468VerificationHistoryIntegrityError(
                "M4.69 verification-history integrity fingerprint mismatch"
            )
...
def build_historical_m4_68_verification_history_integrity(
    records: list[IndependentHistoricalM466VerificationPersistenceReceiptRecord],
