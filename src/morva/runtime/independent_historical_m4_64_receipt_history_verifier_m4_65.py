...
class IndependentHistoricalM464ReceiptHistoryIntegrityError(ValueError):
    """Raised when an M4.64 receipt-history snapshot fails independent verification."""


@dataclass(frozen=True, slots=True)
class IndependentHistoricalM464ReceiptHistoryIntegrity:
    snapshot_id: UUID
    ...
    def __post_init__(self) -> None:
        for name, value in (
            ...
            if len(value) != 64 or any(
                ...
                raise IndependentHistoricalM464ReceiptHistoryIntegrityError(
        ...
        if self.verification_fingerprint.lower() != expected:
            raise IndependentHistoricalM464ReceiptHistoryIntegrityError(
                "independent M4.64 receipt-history verification fingerprint mismatch"
            )
...
def independently_verify_historical_m4_64_receipt_history_integrity(
    *,
    ...
    except HistoricalM463ReceiptHistoryIntegrityError as exc:
        raise IndependentHistoricalM464ReceiptHistoryIntegrityError(
            "persisted M4.64 receipt-history integrity snapshot is structurally invalid"
        ) from exc
    ...
    for record in ordered:
        try:
            ...
            raise IndependentHistoricalM464ReceiptHistoryIntegrityError(
                "M4.63 source verification receipt is structurally invalid"
            ) from exc
    ...
    if persisted.integrity_version != 1:
        blockers.append("M464_INTEGRITY_VERSION_MISMATCH")
    if persisted.history_fingerprint.lower() != reconstructed_history_fingerprint:
        blockers.append("M464_HISTORY_FINGERPRINT_MISMATCH")
    if persisted.record_count != reconstructed_record_count:
        blockers.append("M464_RECORD_COUNT_MISMATCH")
    if persisted.valid_count != valid_count:
        blockers.append("M464_VALID_COUNT_MISMATCH")
    if persisted.fingerprint.lower() != reconstructed_fingerprint:
        blockers.append("M464_INTEGRITY_FINGERPRINT_MISMATCH")
