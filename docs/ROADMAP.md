...
**Current position:** enterprise validation candidate; M4.47 persists an aggregate integrity snapshot over the M4.46 independent historical freshness verification history while preserving the governance/readiness-only boundary; not production-certified for real payroll/payment.
...
1. Keep exact `main` head green across compilation, Ruff, PostgreSQL migrations, pytest, pip-audit and web build; the active development line now extends the M4.30 freshness-policy registry through M4.47 persisted verification-history integrity snapshots.
