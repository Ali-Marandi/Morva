...
**Current position:** enterprise validation candidate; M4.21 independently verifies the M4.20 integration-execution readiness assessment; not production-certified for real payroll/payment.
...
1. Keep exact `main` head green across compilation, Ruff, PostgreSQL migrations, pytest, pip-audit and web build; M4.21 is the current main evidence/integration baseline.
2. Maintain the M4.1–M4.21 evidence/integration line as the software baseline; real authoritative artifacts and staging/pilot execution remain external to the codebase and must be independently supplied, independently verified, approved and validated before any production authority is granted.
