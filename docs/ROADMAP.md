...
1. Keep exact `main` head green across compilation, Ruff, PostgreSQL migrations, pytest, pip-audit and web build; The active development line now extends the M4.30 freshness-policy registry through M4.42 historical snapshot-bound freshness receipt lineage history; the active development line now extends the M4.30 freshness-policy registry through M4.41 historical snapshot-bound freshness receipt lineage.
