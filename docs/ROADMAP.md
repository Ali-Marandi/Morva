...
**Current position:** enterprise validation candidate; M4.23 independently verifies persisted M4.22 readiness receipts through a read-only API; not production-certified for real payroll/payment.
...
- M4.23 independent reconstruction of persisted readiness fingerprints, exact repository/SHA/environment and ready/blocked invariants, plus a ministry-scoped read-only verification API

## Current execution queue

1. Keep exact `main` head green across compilation, Ruff, PostgreSQL migrations, pytest, pip-audit and web build; M4.22 remains the persistence baseline while M4.23 adds independent API-level verification.
