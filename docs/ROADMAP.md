...
**Current position:** enterprise validation candidate; M4.48 independently verifies M4.47 aggregate integrity snapshots over the complete point-in-time M4.46 historical verification history while preserving the governance/readiness-only boundary; not production-certified for real payroll/payment.
...
1. Keep exact `main` head green across compilation, Ruff, PostgreSQL migrations, pytest, pip-audit and web build; the active development line now extends the M4.30 freshness-policy registry through M4.48 independent verification of persisted M4.47 history-integrity snapshots.
